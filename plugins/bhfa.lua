...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '> ['..user_id..' ] Globally Banned from all Groups/SuperGroups With @MegaTesla ')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '> ['..user_id..' ] Un-Globally Banned from all Groups/SuperGroups With @MegaTesla ')
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '> ['..user_id..' ] Globally Banned from all Groups/SuperGroups With @MegaTesla'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
