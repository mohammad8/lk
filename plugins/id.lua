--[[
Add phone by : @MagicCell tnx to : @@Hextor_Admin & MKH
]]
do
local sudo = 87946267 --put your id here(BOT OWNER ID)
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  ...
  local text = 'فامیل :'..(result.from.last_name or '----')..'\n'
               ..'یوزرنیم : '..Username..'\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' or 'آیدی' or 'ایدی' and not matches[2] then
  local receiver = get_receiver(msg)
  local Reply = msg.reply_id
  if msg.reply_id then
  ...
  if msg.from.username then
   ...
   local text = '✏️نام گروه : '..msg.to.title..'\n'
   local text = text..'👥ایدی گروه : '..msg.to.id..'\n'
  ...
  if matches[1]:lower() == 'info' or 'آیدی' or 'ایدی' and matches[2] then
